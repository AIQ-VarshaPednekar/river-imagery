...
# ── PROJ fix — MUST happen before any rasterio import ────────────────────────
# Path confirmed via:
#   python -c "import rasterio, os; print(os.path.join(os.path.dirname(rasterio.__file__), 'proj_data'))"
os.environ["PROJ_DATA"]    = r"C:\Users\ADMIN\miniforge3\envs\river_env\Lib\site-packages\rasterio\proj_data"
os.environ["PROJ_NETWORK"] = "OFF"
os.environ.pop("GDAL_DATA", None)   # remove any stale GDAL_DATA that could conflict
# ─────────────────────────────────────────────────────────────────────────────
...
input_folder  = r"C:\Users\ADMIN\Documents\Scripts\river-imagery\Imagery_Output\Sentinel"
output_folder = r"C:\Users\ADMIN\Documents\Scripts\river-imagery\Imagery_Output\Sentinel_Merged"
...
def merge_tiled(valid_tiles, output_file):
    ...
    CHUNK = 2048  # rows per chunk — reduce to 1024 if out-of-memory
...
def build_overviews(output_file):
    """Build overviews and compute stats using rasterio — no gdalinfo/gdaladdo needed."""
    overview_levels = [2, 4, 8, 16]
    with rasterio.open(output_file, "r+") as ds:
        for i in range(1, ds.count + 1):
            data = ds.read(i, masked=True)
    ...
    print(f"  ✓ Overviews + stats written")
...
for river_name, tiles in river_tiles.items():
    ...
    print(f"  Building overviews + stats for QGIS: {river_name}...")
    try:
        build_overviews(output_file)
    except Exception as e:
        print(f"  ⚠ Overviews failed (non-fatal, file still usable): {e}")
