...
OUTPUT_BASE_FOLDER = r"C:\Users\ADMIN\Documents\Scripts\river-imagery\Imagery_Output"
...
SPECIFIC_RIVERS = ["Alaknanda", "Bhagirathi"]
...
def main():
    ...
    print(f"  ✓ Loaded {len(all_rivers_gdf)} rivers from shapefile")
    print(f"  Available columns: {list(all_rivers_gdf.columns)}")

    # Find the name column (try common variants)
    name_column = None
    for col in ["name", "NAME", "rivname", "rivname", "RIVNAME", "RiverName", "River_Name", "river_name"]:
        if col in all_rivers_gdf.columns:
            name_column = col
            print(f"  Using column '{col}' for river names")
            break
    
    if name_column is None:
        raise ValueError(f"Could not find river name column in shapefile. Available columns: {list(all_rivers_gdf.columns)}")
    
    # Rename to 'name' if needed
    if name_column != "name":
        all_rivers_gdf = all_rivers_gdf.rename(columns={name_column: "name"})
